...
namespace robot_self_filter
{
  ...
  public:
    ...
    void initSelfFilter()
    {
      ...
      // (디버그용) 터미널에 URDF 내용 전체를 출력해 보고 싶다면 주석 해제
      ...
      std::vector<std::string> dbg_links;
      if (this->get_parameter("self_see_links.names", dbg_links)) {
        RCLCPP_INFO(this->get_logger(),
                    "[DBG] self_see_links.names has %zu items", dbg_links.size());
      } else {
        RCLCPP_WARN(this->get_logger(),
                    "[DBG] self_see_links.names parameter NOT found");
      }
      // above is for debugging
      self_filter_->getLinkNames(frames_);
      // BH TEMP (250515) — fmt 없이 std::stringstream 사용
      {
        std::stringstream ss;
        ...
        RCLCPP_INFO_STREAM(this->get_logger(),
          "Loaded link names (" << frames_.size() << "): " << ss.str());
        // std::cout << "temp debugging..." << std::endl;
        ...
        switch (sensor_type_)
        {
          case SensorType::XYZSensor:
          {
            auto sf = std::dynamic_pointer_cast<filters::SelfFilter<pcl::PointXYZ>>(self_filter_);
            if (sf) body_count = sf->getSelfMaskPtr()->getBodies().size();
            break;
          }
          case SensorType::XYZRGBSensor:
          {
            auto sf = std::dynamic_pointer_cast<filters::SelfFilter<pcl::PointXYZRGB>>(self_filter_);
            if (sf) body_count = sf->getSelfMaskPtr()->getBodies().size();
            break;
          }
          // 나머지 센서 타입도 동일하게…
        }
        RCLCPP_INFO(get_logger(), "SelfMask bodies = %zu", body_count);
        }
