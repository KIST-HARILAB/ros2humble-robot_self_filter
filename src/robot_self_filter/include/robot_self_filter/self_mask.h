...
// debugging
#include <geometric_shapes/shapes.h>          // Shape / Mesh 정의 (먼저)
#include <geometric_shapes/mesh_operations.h> // ② createMeshFromResource 등
#include <geometric_shapes/shape_operations.h> // createMeshFromResource (나중)
//
#include <geometric_shapes/bodies.h>   // 전역 bodies 선언
  ...
  switch (geom->type)
  {
    ...
    case urdf::Geometry::MESH:
    {
      ...
      if (mesh && !mesh->filename.empty())
      {
        ...
        // debugging
        {
          res = retriever.get(mesh->filename);                     // ① 성공 시
          RCLCPP_INFO(rclcpp::get_logger("self_mask"),
                      "[SelfMask] got '%s'  (%zu bytes)",
                      mesh->filename.c_str(), res.size);
        }
        catch (const std::exception &e)                            // ② 실패 시
        {
          RCLCPP_ERROR(rclcpp::get_logger("self_mask"),
                      "[SelfMask] resource_retriever ERROR: %s",
                      e.what());
          return nullptr;
        ...
        if (!result)
          RCLCPP_ERROR(rclcpp::get_logger("self_mask"),
                        "[SelfMask] shape creation failed for '%s'",
                        mesh->filename.c_str());
                        // 
      }
      break;
...
protected:
  ...
  bool configure(const std::vector<LinkInfo> &links)
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("self_mask"), "Robot model not found!");
      return false;
    }
    auto urdfModel = std::make_shared<urdf::Model>();
    if (!urdfModel->initString(content))
    {
      RCLCPP_ERROR(rclcpp::get_logger("self_mask"), "Unable to parse URDF!");
      return false;
    }
